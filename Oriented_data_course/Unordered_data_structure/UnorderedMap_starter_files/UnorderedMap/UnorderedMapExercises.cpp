...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  int return_value = fallbackVal;
  // =================================================
  // EXERCISE 2 WORKSPACE: YOUR CODE HERE
  // =================================================
  std::unordered_map<std::string,int>::const_iterator map_it = wordcount_map.find(key);

  return_value = (map_it!= wordcount_map.end())? map_it->second : fallbackVal;

  return return_value; // Change this!
}
...
int memoizedLongestPalindromeLength(LengthMemo& memo, const std::string& str, int leftLimit, int rightLimit, timeUnit startTime, double maxDuration) {
  ...
  //
  return memo[std::make_pair(leftLimit,rightLimit)] = greaterResult; // Hint: You need to change this!
  // =======================================================================
}
