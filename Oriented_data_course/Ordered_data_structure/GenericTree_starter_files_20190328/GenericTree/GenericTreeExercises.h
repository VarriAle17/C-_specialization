...
static void treeFactory(GenericTree<int>& tree) {
  typedef typename GenericTree<int>::TreeNode TreeNode;
  tree.deleteSubtree(tree.getRootPtr());
  TreeNode* tree_root = tree.createRoot(4);
  TreeNode* f_left_most = tree_root->addChild(8);
  tree_root->addChild(15);
  TreeNode* s_left_most = f_left_most->addChild(16);
  f_left_most->addChild(23);
  s_left_most->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  //typedef typename GenericTree<T>::TreeNode TreeNode;
  ...
  _add_child(rootNodePtr->childrenPtrs, results);
...
template<class T>
void _add_child(std::vector<typename GenericTree<T>::TreeNode*> child_node, std::vector<T>& results){
using TreeNode = typename GenericTree<T>::TreeNode;
if (child_node.empty()) return;
std::vector<TreeNode*> new_child_node;
for(auto& a : child_node){
  results.push_back(a->data);
  if (!a->childrenPtrs.empty()) new_child_node.insert(new_child_node.end(), a->childrenPtrs.begin(), a->childrenPtrs.end()); 
}
_add_child(new_child_node, results);
}
