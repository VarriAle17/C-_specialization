...
void LinkedList<T>::insertOrdered(const T& newData) {
  Node* p = this->head_;
    ...
    if(this->empty() || head_->data > newData){
      pushFront(newData);
      return;
    }else p = p->next;

    while(p!=nullptr && p->data < newData){
        p = p->next;
    }
  ...
  // above and in the instructions PDF.make
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  Node* left_pointer = nullptr;
  Node* right_pointer = nullptr;
  
  if(left.empty() && !right.empty()) merged =right;
  else if(!left.empty() && right.empty()) merged = left;
  else{
    left_pointer = left.head_;
    right_pointer = right.head_;
    while(left_pointer!=nullptr && right_pointer!=nullptr){
        if(left_pointer->data<=right_pointer->data){
          merged.pushBack(left_pointer->data);
          left_pointer=left_pointer->next;
        }else{
          merged.pushBack(right_pointer->data);
